...
class OAuthSession:
    def __init__(
        ...
        self.live_session_token_expiration = (
            int(live_session_token_expiration)
            if live_session_token_expiration is not None
            else None
        )
        self.__base_url = "https://api.ibkr.com/v1/api/"
    ...
    def request_live_session_token(self):
        ...
        if self.live_session_token is not None and self.__is_valid_live_session_token():
            print(
                f"Using existing live session token: {self.live_session_token} expires: {self.live_session_token_expiration}",
                end="\n\n",
            )
            return None
        request_url = urljoin(self.__base_url, ENDPOINT)
        request_headers = self.__generate_request_headers(ENCRYPTION_METHOD)
        dh_random = generate_dh_random_bytes()
        request_headers["diffie_hellman_challenge"] = generate_dh_challenge(
            self.dh_prime, dh_random
        )
        ...
        base_string = generate_base_string(
            REQUEST_METHOD, request_url, request_headers, prepend=prepend
        )
        ...
        print(
            f"Generated new live session token: {self.live_session_token} expires: {self.live_session_token_expiration}",
            end="\n\n",
        )
    ...
    def get_secdef_by_conid(self, conid_list: list[int]):
        ...
        request_url = urljoin(self.__base_url, ENDPOINT)
        request_headers = self.__generate_request_headers()
        request_params = {"conids": ",".join([str(conid) for conid in conid_list])}
        base_string = generate_base_string(
            REQUEST_METHOD, request_url, request_headers, request_params
        )
        signature = generate_hmac_sha_256_signature(
            base_string, self.live_session_token
        )
    ...
    def get_stocks_by_symbol(self, symbol_list: list[str]):
        ...
        request_url = urljoin(self.__base_url, ENDPOINT)
        request_headers = self.__generate_request_headers()
        request_params = {"symbols": ",".join(symbol_list)}
        base_string = generate_base_string(
            REQUEST_METHOD, request_url, request_headers, request_params
        )
        signature = generate_hmac_sha_256_signature(
    ...
    def get_list_of_md_subscriptions(self):
        ...
        """
        ENDPOINT = "ibcust/marketdata/subscriptions"
        REQUEST_METHOD = "GET"
        request_url = urljoin(self.__base_url, ENDPOINT)
        request_headers = self.__generate_request_headers()
        base_string = generate_base_string(REQUEST_METHOD, request_url, request_headers)
    ...
    def init_brokerage_session(self, compete: bool = True, publish: bool = True):
        ...
        if self.live_session_token is None:
            raise Exception(
                "No LST found. You need to request an LST first, before attempting to open a brokerage session."
            )
        ENDPOINT = "iserver/auth/ssodh/init"
        REQUEST_METHOD = "POST"
        request_url = urljoin(self.__base_url, ENDPOINT)
        request_headers = self.__generate_request_headers()
        query_params = {"publish": publish, "compete": compete}
        base_string = generate_base_string(
            REQUEST_METHOD, request_url, request_headers, query_params
        )
        signature = generate_hmac_sha_256_signature(
            base_string, self.live_session_token
        )
    ...
    def get_historical_md_data(self, conid: int, period: str = "1w", bar: str = "1d"):
        ...
        request_url = urljoin(self.__base_url, ENDPOINT)
        request_headers = self.__generate_request_headers()
        request_params = {"conid": conid, "period": period, "bar": bar}
        base_string = generate_base_string(
            REQUEST_METHOD, request_url, request_headers, request_params
        )
        signature = generate_hmac_sha_256_signature(
